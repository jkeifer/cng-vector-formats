...
# %%
s3_url_prefix = (
    's3://overturemaps-us-west-2/release/2025-09-24.0/theme=buildings/type=building/'
)
...
#
# Let's see how we can use `AsyncHttpFile` with `ParquetFile` and some of what the latter exposes. Note that creating the `ParquetFile` instances has to make many random reads within the file, so this is not a fast process, especially over the network (typically taking around 20 to 60 seconds).
...
async with AsyncExitStack() as stack:
    for url in urls_that_intersect:
        f = await stack.enter_async_context(
            AsyncHttpFile(
                url,
                # this is to work around slowness in Codespaces
                session_kwargs={'connector': aiohttp.TCPConnector(limit=60)},
            ),
        )
